...
/**
 * A SWF specific implementation of Java2D's <code>Graphics2D</code> API. Calls to
 * this class are converted into a <code>TagList</code> that can be used to
 * construct a SWF Sprite.
 *
...
public class SpriteGraphics2D extends AbstractGraphics2D
{
	...
	public boolean drawImage(Image image, AffineTransform at, ImageObserver obs)
	{
		if (image != null)
		{
			DefineBitsLossless defineBits = DefineBitsLosslessBuilder.build(new LosslessImage(image));
	...
	public void drawJPEG(InputStream inputStream, int length, int width, int height, String name) throws IOException
	{
        if (inputStream != null)
		{
			try
			{
				DefineBits defineBits = DefineBitsBuilder.build(new JPEGImage(inputStream, length));
