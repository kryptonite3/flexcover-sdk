...
final class PersistenceStore
{
    ...
    PersistenceStore(RandomAccessFile file, FontManager fontManager)
    {
        assert file != null;
	          ...
	          String description) throws IOException
	{
		...
		{
			writeSwcFileChecksums(swcFileChecksums, pool, cs2);
		}
	...
	private void writeSwcFileChecksums(Map swcFileChecksums,
									   Map pool,
									   OutputStream cs2) throws IOException
	{
		for (Iterator i = swcFileChecksums.keySet().iterator(); i.hasNext(); )
		{
			String fileName = (String) i.next();
			Long ts = (Long) swcFileChecksums.get(fileName);
			writeU32(cs2, addString(pool, fileName));
			writeLong(cs2, ts.longValue());
		}
	}
	...
	int read(FileSpec fileSpec,
	         ...
	         int[] checksums,
	         Map swcDefSignatureChecksums,
	         Map swcFileChecksums) throws IOException
	{
		if (!readVersion())
		{
		...
		if (!readSwcFileChecksums(pool, swcFileChecksums))
		{
			return -8;
		}
	...
	private boolean readSwcFileChecksums(Object[] pool, Map swcFileChecksums) throws IOException
	{
		int size = readU32();
		...
		for (int i = 0; i < size; i++)
		{
			...
			long ts = readLong();
			if (swcFileChecksums != null) swcFileChecksums.put(fileName, new Long(ts));
		}
