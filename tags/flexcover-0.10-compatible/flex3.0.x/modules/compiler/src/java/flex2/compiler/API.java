...
//	private static final int resolveExpression          = (1 << 12);
								  ...
								  List targets, SymbolTable symbolTable, Configuration configuration)
	{
		...
		for (int i = sources.size() - 1; i >= 0; i--)
		{
			...
			else if ((w & analyze2) == 0)
			{
				...
				boolean okay =  (s.getLogger() == null || s.getLogger().errorCount() == 0) &&
								checkInheritance(u, u.inheritance, symbolTable, processed) &&
								// check(u, u.inheritance, symbolTable, analyze2) &&
			...
			else if ((w & analyze3) == 0)
			{
				...
				boolean okay =  (s.getLogger() == null || s.getLogger().errorCount() == 0) &&
								check(u, u.inheritance, symbolTable, analyze3) &&
			...
			else if ((w & analyze4) == 0)
			{
				...
				boolean okay =  (s.getLogger() == null || s.getLogger().errorCount() == 0) &&
								check(u, u.inheritance, symbolTable, analyze4) &&
	...
	private static boolean checkInheritance(CompilationUnit unit, MultiNameSet types, SymbolTable symbolTable, Set processed)
	{
		// 1. inheritance
		int id = types.getId(), j = id - 1, k = 1;
		int mask = 1 << (j * 4 + k);
		...
		if (!check(unit, types, symbolTable, analyze2))
		{
			return false;
		}
		...
		for (Iterator i = types.iterator(); i.hasNext();)
		{
			...
			if (name instanceof flex2.compiler.util.QName)
			{
				...
				if (!checkInheritance(u, u.inheritance, symbolTable, processed))
				{
					return false;
				}
			}
	...
	public static int validateCompilationUnits(FileSpec fileSpec, SourceList sourceList, SourcePath sourcePath,
											   ...
											   CompilerSwcContext swcContext, Object perCompileData,
											   boolean recompile, Configuration configuration)
	{
		...
		if (perCompileData != null)
		{
			...
			{
				String ns = (String) i.next();
				((macromedia.asc.util.ContextStatics) perCompileData).removeNamespace(ns);
			}
		}
	...
	public static void persistCompilationUnits(FileSpec fileSpec, SourceList sourceList, SourcePath sourcePath,
											   ResourceContainer resources, ResourceBundlePath bundlePath,
											   int checksum, String description, RandomAccessFile f)
		throws IOException
	{
		persistCompilationUnits(fileSpec, sourceList, sourcePath, resources, bundlePath, null, null,
	...
	public static void persistCompilationUnits(FileSpec fileSpec, SourceList sourceList, SourcePath sourcePath,
		...
		throws IOException
	{
		PersistenceStore store = new PersistenceStore(f);
		int count = -1;
		try
		{
			count = store.write(fileSpec, sourceList, sourcePath, resources, bundlePath, sources, units,
								...
								swcDefSignatureChecksums, swcFileChecksums, description);
		}
		finally
		{
    ...
    public static void loadCompilationUnits(FileSpec fileSpec, SourceList sourceList, SourcePath sourcePath,
                ResourceContainer resources, ResourceBundlePath bundlePath,
                List sources, List units,
                int[] checksums, Map swcDefSignatureChecksums, Map swcFileChecksums,
                RandomAccessFile f, String cacheName) throws IOException
    {
        loadCompilationUnits(fileSpec, sourceList, sourcePath,
                resources, bundlePath, sources, units, checksums, 
                swcDefSignatureChecksums, swcFileChecksums, f, cacheName, null);
    }
	...
	public static void loadCompilationUnits(FileSpec fileSpec, SourceList sourceList, SourcePath sourcePath,
		...
		throws IOException
	{
		loadCompilationUnits(fileSpec, sourceList, sourcePath, resources, bundlePath, null, null,
							 new int[] {checksum, checksum, checksum, checksum}, null, null, f, cacheName, null);
	}
	...
	{
		LocalizationManager l10n = ThreadLocalToolkit.getLocalizationManager();
		PersistenceStore store = new PersistenceStore(f, fontManager);
		int count = -1;
		try
		{
			if ((count = store.read(fileSpec, sourceList, sourcePath, resources, bundlePath, sources, units,
									checksums, swcDefSignatureChecksums, swcFileChecksums)) < 0)
			{
